...
import { boxShadows, fontSizes, minWidth, themableColors, uiPalette } from "./foundation";
import { addPrefix, flattenThemeObject, getColorString, isBaseTheme } from "./utils";
import type { ConfigTheme, ConfigThemes, DefaultThemeType, UiOptions } from "./utils/types";
  ...
  Object.keys(themes).forEach((themeName) => {
    ...
    Object.keys(flatColors).forEach((colorName) => {
      ...
      try {
        ...
        const uiColorVar = `--${prefix}-${colorName}`;
        const uiOpacityVar = `--${prefix}-${colorName}-opacity`;

        // Set the css variable in "@layer utilities"
        resolved.utilities[cssSelector]![uiColorVar] = `${h} ${s}% ${l}%`;

        // If an alpha value was provided in the color definition, store it in a css variable
        if (typeof defaultAlphaValue === "number") {
          resolved.utilities[cssSelector]![uiOpacityVar] = defaultAlphaValue.toFixed(2);
        }

        // Set the dynamic color in tailwind config theme.colors
        resolved.colors[colorName] = ({ opacityVariable, opacityValue }) =>
          getColorString(uiColorVar, uiOpacityVar, opacityValue, opacityVariable);
      } catch (error: unknown) {
        if (error instanceof Error) {
          // eslint-disable-next-line no-console
          console.warn("ui-tw-plugin-error", error.message);
        } else {
          // eslint-disable-next-line no-console
          console.warn("ui-tw-plugin-error", error);
        }
      }
    });
  });
...
const corePlugin = (
  ...
  defaultTheme: DefaultThemeType,
  prefix: string,
  fontSmooth: UiOptions["fontSmooth"]
) => {
  const resolved = resolveConfig(themes, defaultTheme, prefix);

  const prefixedBaseColors = addPrefix(uiPalette, "ui");
  const prefixedBoxShadows = addPrefix(boxShadows, "ui");
  ...
  return plugin(
    ({ addBase, addUtilities, addVariant, matchUtilities, theme }) => {
      addBase({
        ":root": {
          "--ui-font-smooth--webkit": fontSmooth === "antialiased" ? "antialiased" : "unset",
          "--ui-font-smooth--moz": fontSmooth === "antialiased" ? "grayscale" : "unset",
        },
      ...
      addUtilities({
        ...resolved.utilities,
        ".ui-antialiased": {
          "-webkit-font-smoothing": "var(--ui-font-smooth--webkit)",
          "-moz-osx-font-smoothing": "var(--ui-font-smooth--moz)",
        },
      });
    ...
    // Extend the Tailwind config
    {
      theme: {
        extend: {
          ...
          padding: {
            "2px": "calc(2px - var(--ui-border-width, 0px))",
            "4px": "calc(4px - var(--ui-border-width, 0px))",
            "6px": "calc(6px - var(--ui-border-width, 0px))",
            "8px": "calc(8px - var(--ui-border-width, 0px))",
            "12px": "calc(12px - var(--ui-border-width, 0px))",
            "14px": "14px",
            "16px": "calc(16px - var(--ui-border-width, 0px))",
          },
          outlineOffset: {
            3: "3px",
          },
          ...
          animation: {
            "ui-fade-in-up": `fadeInUp 0.3s ${animationEasing}`,
            "ui-fade-in-down": `fadeInDown 0.3s ${animationEasing}`,
            "ui-fade-in-left": `fadeInLeft 0.3s ${animationEasing}`,
            "ui-fade-in-right": `fadeInRight 0.3s ${animationEasing}`,
            "ui-fade-out": `fadeOut 0.15s ${animationEasing}`,
            "ui-line-spinner": "lineSpinner 1.5s ease-in-out infinite both",
          },
          keyframes: {
            fadeInUp: {
              "0%": {
            ...
            lineSpinner: {
              "0%, 25%": {
                strokeDashoffset: "var(--ui-dashoffset-97)",
                transform: "rotate(0)",
              },

              "50%, 75%": {
                strokeDashoffset: "var(--ui-dashoffset-25)",
 ...
 */
export const uiTW = (config: UiOptions = {}): ReturnType<typeof plugin> => {
  const {
